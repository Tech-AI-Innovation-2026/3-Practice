...
목표는 LLM 답변 생성이 아닙니다. **SciFact 및 추가 문서를 검색해서 관련 문서의 `doc_id`를 상위 rank에 올리는 retrieval 성능**을 평가합니다.
...
`corpus.jsonl`은 JSON Lines 형식이며, 한 줄이 문서 하나입니다. 대부분의 SciFact 문서는 `title`과 `text`가 바로 들어 있습니다.
