...
- 현재 SciFact보다 실습 난이도를 높일 수 있는 데이터셋 또는 평가 문제 추천
  - 데이터셋 추천은 파일 포맷을 파싱하기 어려운지보다, retrieval 관점에서 왜 어려운지 중심으로 작성하세요. 예를 들어 query와 문서의 lexical gap, 긴 문서에서 근거 구간을 찾아야 하는 정도, 유사하지만 정답이 아닌 distractor 문서의 존재, metadata가 검색 성능에 주는 영향, nDCG@10 같은 평가 지표로 난이도를 측정할 수 있는지 등을 설명하세요.
